...
class BrushlessMotor:
    ...
    def set_speed(self, speed):
        ...
        if self.motor:
            # For forward motion: 0.075 to 0.10
            # For backward motion: 0.025 to 0.05
            # Neutral point is around 0.06
            
            if speed > 0:  # Forward
                # Scale from 0.075 (min) to 0.10 (max) for forward
                pwm_value = 0.075 + (speed * 0.025)
            elif speed < 0:  # Backward
                # Scale from 0.05 (min) to 0.025 (max) for backward
                pwm_value = 0.05 + (speed * 0.025)
            else:  # Stop
                pwm_value = 0.06  # Neutral point
                
            pwm_value = max(0.025, min(0.10, pwm_value))  # Safety clamp
